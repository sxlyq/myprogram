...
void MainWindow::createMap(){
    ...
    if(isReplayed)
    {
        heroObj->setXPos(initialHeroX);
        heroObj->setYPos(initialHeroY);
        std::vector<std::shared_ptr<Enemy>>::iterator p;
        int index = 0;
        for(p=enemies.begin();p!=enemies.end();p++)
        {
            worldTiles.at(cols*(*p)->getYPos()+(*p)->getXPos())->setValue(backupTileValue.at(index));
            index++;
        }
    }
    else
    {
        if(backupTileValue.size() > 0)
            backupTileValue.clear();
        initialHeroX = heroObj->getXPos();
        initialHeroY = heroObj->getYPos();
        std::vector<std::shared_ptr<Enemy>>::iterator p;
        for(p=enemies.begin();p!=enemies.end();p++)
        {
...
bool MainWindow::eventFilter(QObject *obj, QEvent *event)
{
    if(obj == sceneMap)
    {
        ...
        if (event->type() == QEvent::KeyPress && !isPaused)
        {
        ...
        if(event->type() == QEvent::GraphicsSceneMousePress && !isPaused)
        {
...
void MainWindow::on_actionPause_Continue_triggered()
{
    isPaused = !isPaused;
    if(isPaused)
        timer->stop();
